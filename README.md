...
| `python/` | **The authoritative implementation going forward** ([#2](https://github.com/ProjectSidewalk/label-latlng-estimation/issues/2)): full port of the analysis (all seven candidate estimators). `gsv_depth.py` + `run_depth_pilot.py` add the issue #4 depth pilot: fetching fresh GSV depth and replicating the 2020 depth→latlng pipeline bit-exactly. `pov_inversion.py` + `run_pov_inversion.py` add the issue #5 exact click→POV inversion (verified against production to ≤1 px) and the era-faithful forward model of the 2017–2020 placement pipeline. |
...
pytest                          # 201 tests: data contract, R↔Python equivalence, findings,
