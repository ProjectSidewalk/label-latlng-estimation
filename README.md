...
| 2026-09-02 | [Production sign-off](reports/2026-09-02-production-signoff.md) | The estimator as SidewalkWebpage ships it (`approximation3`), signed off for production against every method that has stamped `computation_method` (`approximation1` 3.57 m, the regression `approximation2` 1.08 m, `approximation3` 0.40 m on modern truth), winning every zoom, label type, resolution and city (a height calibrated on every *other* city still wins everywhere); on the regression's own 720×480-era split it still edges it (1.38 vs 1.46 m) and beats it by half a metre at equal calibration budget, the gap being the era truth's inflated scale. Geodesy settled (the 6371 km sphere; ≤ 11 cm from WGS84 at the largest answer), a 59-case fixture pinning the Scala, JS and SQL implementations to 1e-9°, the frame contract Immersive Explore needs, and a rig-tilt rider that bounds the pitch/roll question the 2020–2022 crop work left open at a few percent of the estimator's variance. ([SidewalkWebpage#5084](https://github.com/ProjectSidewalk/SidewalkWebpage/issues/5084)) |
...
| `run_signoff.py` (+ `signoff.py`, `signoff_figures.py`) | Production sign-off of the shipped estimator: both truth frames head-to-head with the 2021 regression, hold-out and leave-one-city-out calibration checks, the rig-tilt rider, geodesy, the viewport frame contract, the cross-implementation parity fixture, and the worked examples. | [SidewalkWebpage#5084](https://github.com/ProjectSidewalk/SidewalkWebpage/issues/5084) |
